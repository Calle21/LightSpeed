...
lex' :: Sweep
lex' path (Input s) = Lexed $ loop 1 1 s
  where
  loop :: Int -> Int -> C.ByteString -> [Tok2]
  loop col line inp
    | C.null inp = []
    | otherwise  = if col == 1 && synMultiline inp then multiLine (line + 1) $ fromJust $ dropLine inp
                          ...
                          c    -> if C.pack "\\\\" `C.isPrefixOf` inp
                                  then case dropLine inp of
                                         Nothing   -> []
                                         Just inp' -> loop 1 (line + 1) inp'
                                  else let (tok, len, inp') = token c
                                       in (col, line, tok) : loop (col + len) line inp'
    where
    multiLine :: Int -> C.ByteString -> [Tok2]
                ...
                '\'' -> let (s', inp') = C.span (symChar ||| punctuation) (C.tail inp)
                            c | C.length s' == 1 = C.head s'
                              ...
                              | C.null s'        = if C.null inp'
                                                   then lError (col + 1) line path "File ended after char character (')"
                                                   else if C.head inp' == ' '
                                                        then ' '
                                                        else lError (col + 1) line path "Expected a space"
